...
import { IAccountBalance, IServerSideDatasourceWithCRUD, IFormSubmitHandler } from "./interfaces";
...
const App: FunctionComponent = (): React.ReactElement => {
  ...
  const addRow: IFormSubmitHandler = (data: IAccountBalance) => {
    datasource
      .createRow(data)
      .then(() => {
  ...
  const updateRow: IFormSubmitHandler = (data: IAccountBalance) => {
    datasource
      .updateRow(data)
      .then(() => {
        gridApi.purgeServerSideCache();
  ...
  const updateSelectedRowHandler: React.MouseEventHandler<HTMLButtonElement> = () => {
    ...
    if (selectedNode) {
      ...
      datasource
        ...
        .then((selectedRow: IAccountBalance) => {
  ...
  const openForm = (data: IAccountBalance, fn: IFormSubmitHandler) => {
    setFormData(data);
  ...
  return (
    <div className="container my-4">
      ...
      {showForm ? <AccountBalanceForm data={formData} submit={formSubmitHandler} hide={closeForm} /> : null}
    </div>
  )
}
